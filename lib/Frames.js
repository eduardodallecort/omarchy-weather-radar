...
// Index of the frame published at exactly this moment, or -1. The map's two
// layers each hold the moment they show rather than a position in the list,
// so that a new list, shifted by a frame, does not quietly repoint the layer
// on screen to the frame beside it.
function indexOfTime(frames, time) {
  if (!frames || !(time > 0)) return -1
  for (var i = 0; i < frames.length; i++) {
