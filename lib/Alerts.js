...
// Where the record is kept, or "" when there is nowhere to keep it. Built only
// from an absolute home: an unset one would read as "null" and make the path
// relative to wherever the shell was started, where writing it would create a
// directory named "null". With no file the latch lives in memory, and an
// alert still works for the session; only a reload can repeat one.
function latchFilePath(home) {
  if (typeof home !== "string" || home.charAt(0) !== "/" || /[\x00-\x1f]/.test(home)) return ""
  return home.replace(/\/+$/, "") + "/.local/state/omarchy/weather-radar-alert.json"
}
