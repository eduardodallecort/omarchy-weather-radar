...
function featurePaths(layer, index, view, minStep, breakAtDomainEdge) {
  ...
  for (var r = firstRing; r < lastRing; r++) {
    ...
    var lastX = 0, lastY = 0
    // Outcode of the second-to-last emitted point. Folding is judged against
    // this, because it is where the surviving edge starts.
    var anchorCode = 0
    var lastCode = 0
    ...
    for (var p = from; p < to; p++) {
      ...
      if (!isLast && path.length >= 4 && code !== 0 && (anchorCode & code) !== 0) {
